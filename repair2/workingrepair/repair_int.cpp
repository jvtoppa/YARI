...
struct PairHash
{
    ...
    {
        return hash<uint>()(p.first) ^ (hash<uint>()(p.second) << 1);
    }
};
...
void freeHashTable(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht)
{
    /*
...
void freeAll(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht, vector<PAIRNODE*>& q)
{
    freeHashTable(ht);
    freeBuckets(q);
}
...
void printHashTable(const unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht)
{
...
void printDS(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht, vector<SEQ>& s, vector<PAIRNODE*>& q)
{
    printHashTable(ht);
    printBuckets(q);
    printConnections(s);
...
void insert(PAIR* p, unordered_map<pair<uint, uint>, PAIR*, PairHash>& htp, vector<PAIRNODE*>& heap, uint current_position, vector<SEQ>& s)
{
    /*
...
void firstPass(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht, vector<SEQ>& s, vector<PAIRNODE*>& heap) 
{   
    /*
    ...
    */
    ...
    unordered_map<pair<uint, uint>, uint, PairHash> last_occurrence;
    
    for (uint i = 0; i < string_size - 1; i++) 
    {
        pair<uint, uint> current_pair = {s[i].code, s[i+1].code};
...
void findAdjPairs(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht, vector<SEQ>& s, vector<uint>& pairPos, uint position)
{
    /*
    ...
    */
    ...
    uint leftuintPos = prev(s, position);
    if(leftuintPos != N)
    pairPos.push_back(leftuintPos);
    uint rightuintPos = next(s, position);
    if(rightuintPos != N)
    pairPos.push_back(rightuintPos);
...
void decreaseAdjToPair(
    unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht,
    vector<SEQ>& s,
    ...
    vector<PAIRNODE*>& q)
{
    ...
    s[actualNext].code = static_cast<uint>(N);
    s[actualNext].prev = position;
    s[actualNext].next = nextNext;
    

    uint old_uint = s[position].code;
...
void compactify(unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht,
vector<SEQ>& s,
vector<PAIRNODE*>& q)
{
    ...
    for (auto& pair_entry : ht) {
        ...
        if (posMap.count(p->b_pos))
         ...
         s[p->b_pos].code = static_cast<uint>(N);
    }
...
void repair(
    unordered_map<pair<uint, uint>, PAIR*, PairHash>& ht,
    vector<SEQ>& s,
    vector<PAIRNODE*>& q, vector<uint>& history)
{
    uint nextRule = 256;
    vector<uint> ruleHistory;
    uint iterator = 0;
    uint itert = 0;
    ...
    while (true)
    {
        ...
        */
        ...
        pair<uint, uint> key = {mostFreqPair->left, mostFreqPair->right};
       ...
       // printDS(ht, s, q);
        for (int i = 0; i < s.size(); i++)
        {
            if(s[i].code != N)
            {
                if(s[i].code < 128)
                {
                    cout << static_cast<char>(s[i].code);
                }
                else
                {
